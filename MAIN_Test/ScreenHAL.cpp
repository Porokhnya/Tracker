...
void HalDC::initHAL()
{
  //Тут инициализация/переинициализация дисплея
    ...
    /*  halDCDescriptor->InitLCD();
      ...
      #endif
    //  // setFont(SCREEN_SMALL_FONT);
	   //halDCDescriptor->print("Test", 0, 0);
	   //SerialUSB.println("++InitLCD++");
	   //delay(2000);
	 #else
...
void HalDC::setup()
{
  //создание библиотеки для экрана
   ...
   // halDCDescriptor = new LCD5110(NOKIA_SCK_PIN,NOKIA_MOSI_PIN,NOKIA_DC_PIN,NOKIA_RST_PIN,NOKIA_CS_PIN);
	halDCDescriptor = new LCD5110(7, A4, 5, 30, 31);
	halDCDescriptor->InitLCD();
	halDCDescriptor->setContrast(40);
	halDCDescriptor->setFont(SmallFont);
	halDCDescriptor->print("Test", 0, 0);
	halDCDescriptor->update();
	delay(2000);
	/*
	LCD5110 myGLCD(7, A4, 5, 30, 31);
	myGLCD.InitLCD();
	myGLCD.setFont(SmallFont);
	myGLCD.print("LCD5110_Graph", 0, 0);
	myGLCD.update();
	SerialUSB.println("++InitLCD++");
	delay(2000);
*/
