...
class CoreESPTransport : public CoreTransport
{
  ...
  public:
    ...
    // Возвращает ссылку на активный транспорт, БЕЗ изменения счётчика ссылок. 
    // Возвращается NULL, если транспорт никем не создан.
    // Этот метод используется в loop, чтобы обновлять состояние конечного автомата ESP,
    // другие части кода не должны трогать этот метод, используя вместо него пару Create/Destroy.
    static CoreESPTransport* ActiveInstance();
    
    // Cоздаёт экземпляр транспорта, увеличивая счётчик ссылок. 
    // Если транспорт только создался - включается питание ESP и транспорт начинает работу. 
    // Если транспорт уже был - увеличиваетс счётчик ссылок, и возвращается ссылка на транспорт.
    // Этот метод должны использовать все, заинтересованные в ESP клиенты.
    static CoreESPTransport* Create();

    // Освобождает транспорт, уменьшая счётчик ссылок. 
    // Если на транспорт никто не ссылается - он удаляется из памяти, для ESP выключается питание.
    // Этот метод должны использовать все, заинтересованные в ESP клиенты.
    void Destroy(); 
