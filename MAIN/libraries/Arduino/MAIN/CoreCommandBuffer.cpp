...
const char DRIVER_COMMAND [] PROGMEM = "DRIVER"; //ФИО водителя до 70 символов
const char VEHICLENUMBER_COMMAND [] PROGMEM = "VEHICLENUMBER"; //Гос.номер ТС до 10 символов
const char TRAILERNUMBER_COMMAND [] PROGMEM = "TRAILERNUMBER"; //Гос.номер прицепа до 10 символов
...
void CommandHandlerClass::processCommand(const String& command,Stream* pStream)
{
    ...
    if(command.startsWith(CORE_COMMAND_SET))
    {
      ...
      if(cParser.parse(command,true))
      {
        ...
        if(!strcmp_P(commandName, DRIVER_COMMAND))
        {
            if(cParser.argsCount() > 1)
            {
              commandHandled = setDRIVER(cParser, pStream);
            }
            else
            {
              // недостаточно параметров
        ...
        if(!strcmp_P(commandName, VEHICLENUMBER_COMMAND))
        {
            if(cParser.argsCount() > 1)
            {
              commandHandled = setVEHICLENUMBER(cParser, pStream);
            }
            else
            {
              // недостаточно параметров
        ...
        if(!strcmp_P(commandName, TRAILERNUMBER_COMMAND))
        {
            if(cParser.argsCount() > 1)
            {
              commandHandled = setTRAILERNUMBER(cParser, pStream);
            }
            else
            {
              // недостаточно параметров
    ...
    if(command.startsWith(CORE_COMMAND_GET))
    {
      ...
      if(cParser.parse(command,false))
      {
        ...
        if(!strcmp_P(commandName, DRIVER_COMMAND)) // 
        {
            // запросили  GET=DRIVER
            commandHandled = getDRIVER(commandName,cParser,pStream);                    
        } // LS       
        else
        if(!strcmp_P(commandName, VEHICLENUMBER_COMMAND)) // 
        {
            // запросили  GET=CARNUM
            commandHandled = getVEHICLENUMBER(commandName,cParser,pStream);                    
        } // LS 
        else
        if(!strcmp_P(commandName, TRAILERNUMBER_COMMAND)) // 
        {
            // запросили  GET=TRNUM
            commandHandled = getTRAILERNUMBER(commandName,cParser,pStream);                    
        } // LS      
...
//--------------------------------------------------------------------------------------------------------------------------------------
bool CommandHandlerClass::getDRIVER(const char* commandPassed, const CommandParser& parser, Stream* pStream)
{
  if(parser.argsCount() < 1)
    return false;  
  ...
  pStream->print(commandPassed);
  pStream->print(CORE_COMMAND_PARAM_DELIMITER);
  pStream->println( Settings.getDriver());
  

  return true;
}
//--------------------------------------------------------------------------------------------------------------------------------------
bool CommandHandlerClass::setDRIVER(CommandParser& parser, Stream* pStream)
{

  if(parser.argsCount() < 2)
    return false;
  
  String Driver = parser.getArg(1);
  ...
  pStream->print(CORE_COMMAND_PARAM_DELIMITER);
  pStream->println(Driver);

  return true;
}
//--------------------------------------------------------------------------------------------------------------------------------------
bool CommandHandlerClass::getVEHICLENUMBER(const char* commandPassed, const CommandParser& parser, Stream* pStream)
{
  if(parser.argsCount() < 1)
    return false;  
  ...
  pStream->print(commandPassed);
  pStream->print(CORE_COMMAND_PARAM_DELIMITER);
  pStream->println( Settings.getVehicleNumber());
  

  return true;
}
//--------------------------------------------------------------------------------------------------------------------------------------
bool CommandHandlerClass::setVEHICLENUMBER(CommandParser& parser, Stream* pStream)
{

  if(parser.argsCount() < 2)
    return false;
  
  String VehicleNumber = parser.getArg(1);

  Settings.setVehicleNumber(VehicleNumber);
  ...
  pStream->print(parser.getArg(1));
  pStream->print(CORE_COMMAND_PARAM_DELIMITER);
  pStream->println(VehicleNumber);

  return true;
}
//--------------------------------------------------------------------------------------------------------------------------------------
bool CommandHandlerClass::getTRAILERNUMBER(const char* commandPassed, const CommandParser& parser, Stream* pStream)
{
  if(parser.argsCount() < 1)
    return false;  
  ...
  pStream->print(commandPassed);
  pStream->print(CORE_COMMAND_PARAM_DELIMITER);
  pStream->println( Settings.getTrailerNumber());
  

  return true;
}
//--------------------------------------------------------------------------------------------------------------------------------------
bool CommandHandlerClass::setTRAILERNUMBER(CommandParser& parser, Stream* pStream)
{

  if(parser.argsCount() < 2)
    return false;
  
  String TrailerNumber = parser.getArg(1);

  Settings.setTrailerNumber(TrailerNumber);

  pStream->print(CORE_COMMAND_ANSWER_OK);
  pStream->print(parser.getArg(1));
  pStream->print(CORE_COMMAND_PARAM_DELIMITER);
  pStream->println(TrailerNumber);
